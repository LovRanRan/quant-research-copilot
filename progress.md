...
| **Current Sub-step** | `1.3 — DESIGN.md v0.1 FILL IN pass (in progress · 2 / 7 TypedDicts done · 1 worked example + 1 真 hand-write)`                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| **Stage 1 Progress** | 🟩🟩🟩🟩🟩🟩🟩🟩🟩🟩 10 / 10 micro-steps (Claude draft portion)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| **Overall Progress** | 🟦🟦⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜⬜ 1 / 13 stages (Stage 1 pending FILL IN completion)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| **Blocker**          | none                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| **Last Activity**    | 2026-04-24 · **`FilingMeta` TypedDict 完成 · FILL IN pass 第一个真 hand-write**。Paste in DESIGN.md §5.1 (line 246-251)，line 250 旧 inline sketch 已删。3 轮 review 收敛 🟢：第 1 轮 typo + 缺 reasoning 🟡 / 第 2 轮 `optional[index]` 大小写+未定义 🟡 / 第 3 轮 🟢。字段决策：`form: str`（灵活，SEC 表单变化承担）/ `url: Optional[str]` 带语义注释指向 index page / `period_of_report: Optional[date]`（8-K 接口经常 null）。                                                                                                                                                       |
| **Working Mode**     | HAND-WRITE · resume-grade（review + 方向提示，code 全部 Haichuan 写）｜进度：完成驱动，非日历驱动                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| **Next Action**      | Hand-write `FilingSection` 枚举（用于 `get_filing_section(accession_number, section: ...)` 的 `section` 参数）at DESIGN.md §5.1 line 261-264。候选已在 inline 注释列出：`item_1_business / item_1a_risk / item_7_mdna / item_8_financials`。**关键决策**：`str` vs `Literal[...]` vs `enum.Enum`。这次 trade-off **和 `form` 相反** — section 列表是 SEC 法规规定的、长期稳定，受控集合的维护成本很低，所以倾向 `Literal` 或 `Enum`。两种细分差别：`Literal` 轻量（只是 type alias），`Enum` 重一点但带 runtime 安全。完成后贴给 Claude review。完成后顺手把 `get_filing_section` 函数签名里的 `section: str` 改成新类型。 |
...
- [X]  1.1 主 repo 骨架（`projects/quant-research-copilot/`）
...
- [X]  1.2 mcp-servers/ subdirs + 3 MCP skeletons
  - [X]  1.2.a `mcp-servers/mcp-sec-edgar/` ✅ 2026-04-22 (uv workspace auto-enabled, see ADR-003)
  - [X]  1.2.b `mcp-servers/mcp-fred-macro/` (same shape)
  - [X]  1.2.c `mcp-servers/mcp-backtest/` (same shape)
...
| 2026-04-24       | 1.2            | 🔧 Roadmap 复选框漂移修复。1.2 / 1.2.b / 1.2.c 三个框补勾（实际 2026-04-22 已完成，Logs 与磁盘一致，Roadmap 漏勾）。这是没走完 Update Protocol 第 3 步留下的漂移，也是为何需要协议本身。 |
| 2026-04-24       | 1.3            | ✅ **`FilingMeta` TypedDict 完成 — FILL IN pass 第一个真正的 hand-write。** Paste in DESIGN.md §5.1 (line 246-251)；line 250 旧 inline sketch 已删；`# form_type` 文档保留。字段决策：`accession_number: str` / `filed_date: date` / `form: str`（选 str 不选 Literal，理由：SEC 表单类型变化频率不可控）/ `url: Optional[str]` 带 inline 注释 `# index page URL, not primary document` / `period_of_report: Optional[date]`（理由：8-K 突发公告 SEC 接口常返回 null，强制 mandatory 会让下游崩）。Review 历经 3 轮收敛 🟢：第 1 轮 typo `Filling`/`TypeDict` + 缺 reasoning + 缺 Optional → 🟡；第 2 轮 `optional[index]` 大小写 + 用未定义类型 → 🟡；第 3 轮 🟢。**Lessons**：(1) hand-write 首次提交常缺 reasoning，今后明确要求"代码 + reasoning 同时贴"；(2) 用户主动声明"reasoning 自己写"是合规边界，Claude 仅 review 代码层；(3) 字段语义注释（如 `# index page URL`）写在类型旁边比塞进类型本身更干净；(4) cosmetic：两个 top-level class 之间应空一行（black 标准），用户已知，自决何时清理。 |
